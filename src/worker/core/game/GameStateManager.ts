import type { GameAction, GameEvent, GameState, PlayerState } from '@shared/types';
...
export class GameStateManager {
  ...
  async createGame(playerId: string, deckId: string): Promise<GameState> {
    // Get player's deck
    const deck = await this.env.DECKS.get(`${playerId}/${deckId}`);
    if (!deck) {
      throw new HTTPException(400, { message: 'Deck not found' });
    }

    const deckData = JSON.parse(deck);
    const shuffledDeck = this.shuffleArray([...deckData.cards]);
    ...
    const handCards = await Promise.all(
      initialHand.map(async (cardId) => {
        const card = await this.env.CARDS.get(`card:${cardId}`);
